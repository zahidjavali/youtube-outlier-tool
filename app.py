...
def get_channel_stats(_youtube_service, channel_ids):
    """Batch fetch subscriber counts for channels. Underscore prevents cache hashing error."""
    if not channel_ids:
        return {}
    try:
...
def analyze_videos(youtube_service, search_type, query, view_multiplier=100, min_views=100000):
    ...
    try:
        ...
        for v in videos:
            ...
            outlier_score = multiplier_ratio
...
if not st.session_state.api_key_valid:
    ...
    min_views_threshold = st.slider("Min Views", min_value=10000, max_value=1000000, value=100000, step=10000, help="Absolute min views for consideration")
            ...
            with st.spinner("🔄 Fetching 50 videos, channel stats, and analyzing..."):
                ...
                if err:
                    ...
                    if not outliers_df.empty:
                            ...
                            <div class="video-card {is_outlier_class}">
                                <div style="display: flex; gap: 15px;">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <p><strong>Views:</strong> {int(row['views']):,} | <strong>Subs:</strong> {int(row['subscribers']):,} | <strong>Multiplier:</strong> {row['multiplier_ratio']:.0f}x</p>
                        ...
                        st.info("No true outliers found. Try lowering the multiplier threshold or min views.")
                    ...
                    for _, row in df.sort_values("outlier_score", ascending=False).iterrows():
                        ...
                        <div class="video-card {is_outlier_class}" style="font-size: 14px;">
                            <strong><a href="{youtube_url}" target="_blank" style="color: #1da1f2; text-decoration: none;">{row['title'][:80]}...</a></strong> 
                            <a href="{youtube_url}" target="_blank" class="watch-link" style="float: right;">▶ Watch</a><br>
                            Views: {int(row['views']):,} | Subs: {int(row['subscribers']):,} | Multiplier: {row['multiplier_ratio']:.0f}x | 
                    ...
                    with c1:
                        ...
                        chart_data = df[["views", "multiplier_ratio"]].head(20)
