...
<style>
    ...
    .video-result-card.outlier {
        border-color: #FFD700; /* Gold border for outliers */
...
def analyze_videos(youtube_service, search_type, query_input, view_multiplier, min_views, avg_multiplier):
    try:
        # Resolve Channel ID if necessary
        resolved_channel_id = None
        if "channel" in search_type:
            resolved_channel_id = get_channel_id_from_input(youtube_service, query_input)
            if not resolved_channel_id: return None, None, f"❌ Could not find channel for '{query_input}'."

        # Fetch videos based on search type
        order = "date" if search_type == "channel_avg_self" else "viewCount"
        params = {"part": "snippet", "maxResults": 50, "type": "video", "order": order}
        params["channelId"] = resolved_channel_id if "channel" in search_type else None
        params["q"] = query_input if "channel" not in search_type else None
        search_results = youtube_service.search().list(**{k: v for k, v in params.items() if v is not None}).execute().get("items", [])
        ...
        for v in video_details:
            ...
            rows.append({
                "video_id": v.get("id"), "published": pub_date.date(), "title": snip.get("title", "N/A"),
                ...
                "subscribers": channel_info["subscribers"], "channel_avg_views": channel_info["average_views"],
            })
        df = pd.DataFrame(rows)
        if df.empty: return None, None, "No data processed."

        # Calculate Z-Score and Outlier status
        df['z_score'] = ((df['views'] - df['views'].mean()) / df['views'].std()).fillna(0)
        if "avg" in search_type:
            avg = df['views'].mean() if search_type == "channel_avg_self" else df['channel_avg_views']
            df['outlier_score'] = df['views'] / avg.apply(lambda x: max(x, 1))
...
if not st.session_state.api_key_valid:
    ...
    stype_option = st.radio("Mode:", ("Search Term (vs Subs)", "Search Term (vs Channel Avg)", "By Channel (vs Subs)", "By Channel (vs Channel Avg)"), horizontal=True)
    stype_map = {"Search Term (vs Subs)": "search_vs_subs", "Search Term (vs Channel Avg)": "search_vs_avg", "By Channel (vs Subs)": "channel_vs_subs", "By Channel (vs Channel Avg)": "channel_avg_self"}
    ...
    with c2:
        if "avg" in stype_val:
            ...
            min_views = c2.select_slider("Min. Views", [10k for k in range(1,10)] + [100_000, 250_000, 500_000, 1_000_000], value=50_000)
            avg_multiplier = 10
...
if 'query_params' in st.session_state:
    if not st.session_state.query_params[1].strip():
        st.warning("Please enter a search term or channel.")
    else:
        with st.spinner("🔄 Analyzing videos..."):
            ...
            if err: st.error(err)
                ...
                v1, v2 = st.columns(2)
                v1.subheader("View Velocity Over Time")
                v1.line_chart(df.sort_values("published")[["published", "velocity"]].set_index("published"))
                v2.subheader("Top 10 Outliers by Velocity")
                v2.bar_chart(outliers_df.nlargest(10, "velocity")[["title", "velocity"]].set_index("title"))

                # Results List
                st.header("📹 Analyzed Videos")
                for _, row in df.iterrows():
                    outlier_class = "outlier" if row['is_outlier'] else ""
                    st.markdown(f"""
                    <div class="video-result-card {outlier_class}">
                        ...
                        <div class="video-details">
                            <div class="video-title">
                                <a href="https://www.youtube.com/watch?v={row['video_id']}" target="_blank">{row['title']}</a>
                            </div>
                            <div class="video-stats">
                                <span>Published: <strong>{row['published']}</strong></span>
                                <span>Views: <strong>{row['views']:,}</strong></span>
                                <span>Likes: <strong>{row['likes']:,}</strong></span>
                                <span>Comments: <strong>{row['comments']:,}</strong></span>
                                <span>Views/Day: <strong>{row['velocity']:,.0f}</strong></span>
